...
export function isValid(question: Question, answer: string): boolean {
    if (question.type === "short_answer_question") {
        ...
        // For multiple_choice_question, check if the answer is one of the options
        return question.options.includes(answer);
    }

    // Default case: if the type is unknown, return false
    return false;
}
...
export function toShortForm(question: Question): string {
    const nameShort = question.name.substring(0, 10);

    return `${question.id}: ${nameShort}`;
}
