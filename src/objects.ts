...
export function isCorrect(question: Question, answer: string): boolean {
    ...
    const trimmedQuestion = question.expected.trim();
    const trueQuestion = trimmedQuestion.toLowerCase();
    if (userAnswer === trueQuestion) {
        return true;
    } else {
        return false;
    }
...
export function toShortForm(question: Question): string {
    const trimmedQName = question.name.substring(0, 10);
    const qid = question.id;
    return qid + ": " + trimmedQName;
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    ...
    return qDupe; //change back to oldQuestion if test fails
}
